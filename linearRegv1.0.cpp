...
SCSFExport scsf_linearRegv1(SCStudyInterfaceRef sc)
{
    ...
    if(sc.SetDefaults)
    {
        sc.GraphName = "linear regression v1.0";
        sc.GraphRegion = 0;
...
//we will then access these two arrays for consecutive iterations
    ...
    for (int k = startBarIndex, j = 0; k < sc.ArraySize - 1; k++, j++){

        if (i_type.GetIndex() == 0){

            *(pPrice + j) = sc.Open[k];
            *(sArray + j) = sc.BaseDateTimeIn[k].GetTimeInSeconds() / 60;
            ...
            *(pBarRange + j) = sc.High[k] - sc.Low[k];
...
//calculations
        ...
        for (int rangeCounter = 0, p = i; rangeCounter < currentRange; rangeCounter++, p++){

            SSresidual += pow(*(pPrice + p) - (slope * *(sArray + p) + intercept), 2);
            SStotal += pow(*(pPrice + p) - meanY, 2);

            SSdeviations = *(pPrice + p) - (slope * *(sArray + p) + intercept);
...
//break the loop when the minimum r and r2 value is met
        ...
        if (abs(rValue) > i_rMin.GetFloat() && r2Value > i_r2Min.GetFloat()){
            ...
            double pricedif = sc.Close[endBarIndex] - sc.Open[startBarIndex + counter];
            int timedif = sc.BaseDateTimeIn[sc.ArraySize - 1].GetTimeInSeconds() - sc.BaseDateTimeIn[sc.ArraySize - currentRange - i_barsBack.GetInt()].GetTimeInSeconds();
            ActualAngle = atan(abs(pricedif) / (timedif/60)) * 180 / M_PI;
            ...
            predictVal = slope * (secondsArray[userDefinedRange - 1] + i_predict.GetInt()) + intercept;
        ...
        if (currentRange == 11 && tresholdMet == false){
